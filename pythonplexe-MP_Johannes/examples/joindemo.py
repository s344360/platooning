...
import sys
import random
from examples.utils import add_platooning_vehicle_extended, communicate, communicate_2,  get_distance, \
    start_sumo, running
...
# maneuver actors
LEADER = "v.0"
JOIN_POSITION = 5 # Vehicle Position is a front Vehicle
...
def add_vehicles(plexe, n, real_engine=False):
    ...
    vid = "v.%d" % n
    add_platooning_vehicle_extended(plexe, vid, 10, 1, SPEED, DISTANCE, real_engine)
...
def open_gap(plexe, vid, jid, topology, n):
    ...
    plexe.set_path_cacc_parameters(vid, distance=JOIN_DISTANCE*4)
    return topology
...
def main(demo_mode, real_engine, setter=None):
    ...
    while running(demo_mode, step, 600000):
        ...
        if step == 0:
            ...
            PCSCommunication.send_checkin_to_pcs(plexe)#+++added
