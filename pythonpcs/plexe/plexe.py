...
class Plexe(traci.StepListener):
        ...
        if version in self.versions:
            ...
            for k, v in self.versions.items():
                if version.startswith(k):
                    print("startWithK",version.startswith(k))#+++
    ...
    # added
    def get_lane_index(self, vid: int) -> int:
        """
        :param vid: vehicle id
        :return: the lane index of the given vehicle
        """
        return self.plexe.get_lane_index(vid)

    def get_lane_id(self, vid: int) -> str:
        """
        :param vid: vehicle id
        :return: laneID of the given vehicle
        """
        return self.plexe.get_lane_id(vid)

    def get_lane_position(self, vid: int) -> float:
        """
        :param vid: vehicle id
        :return: the lane position of the given vehicle
        """
        return self.plexe.get_lane_position(vid)

    def get_type_id(self, vid: int):
        """
        :param vid: vehicle id
        :return: the id of the type of the given vehicle
        """
        return self.plexe.get_type_id(vid)

    def get_max_speed(self, vid: int):
        """
        :param vid: vehicle id
        :return: maximum speed in m/s of the given vehicle
        """
        return self.plexe.get_max_speed(vid)

    def set_speed(self, vid: int, speed: float):
        """
        set the speed for the named vehicle
        :param vid: vehicle id
        :param speed: speed in m/s
        """
        return self.plexe.set_speed(vid, speed)
